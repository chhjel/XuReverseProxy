...
export default class ManualApprovalProxyAuthPage extends Vue {
  ...
  async mounted() {
    this.service = new ManualApprovalService(
      this.options.client.id,
      this.options.authenticationId,
      this.options.solvedId,
    );
  ...
  async onApproveClicked(): Promise<any> {
    await this.approve();
  }
  async onUnApproveClicked(): Promise<any> {
  ...
  async onUnBlockClicked(): Promise<any> {
    await this.setClientBlocked(false, "");
  }
  async updateClientNote(): Promise<any> {
    await this.setClientNote(this.clientNote);
  }
  ...
  async onBlockIpClicked(): Promise<any> {
    ...
    if (
      this.options.selfIP == this.options.client.ip &&
      !confirm(
        `The IP is the same as your own (${this.options.client.ip}), sure you want to proceed? You will loose access to this interface.`,
      )
    )
      return;
  ...
  get ipContinent(): string | null {
    return this.options.client.ipLocation?.continent || null;
  }
  get ipCountry(): string | null {
    return this.options.client.ipLocation?.country || null;
  }
  get ipCity(): string | null {
    return this.options.client.ipLocation?.city || null;
  }
  get ipFlagUrl(): string | null {
    return this.options.client.ipLocation?.flagUrl || null;
  }
  ...
  get formattedEasyCode(): string {
    if (!this.options.currentChallengeData.easyCode || this.options.currentChallengeData.easyCode.length < 6)
      return this.options.currentChallengeData.easyCode;
    else
      return `${this.options.currentChallengeData.easyCode.substring(
        0,
        2,
      )} ${this.options.currentChallengeData.easyCode.substring(
        2,
        4,
      )} ${this.options.currentChallengeData.easyCode.substring(4, 6)}`;
  }
  ...
  get showAka(): boolean {
    return (
      this.options.proxyConfig.challengeTitle &&
  ...
  <div class="manual-approval-page">
    ...
    <div class="content">
      <div class="header">
        ...
        <div class="aka" v-if="showAka">
          (aka <span>{{ options.proxyConfig.name }}</span
          >)
        </div>
        <div class="link">
          on url <a :href="options.url">{{ options.url }}</a>
        </div>
      </div>
      ...
      <!-- Actions -->
      <div class="actions">
        <button-component v-if="!isApproved" @click="onApproveClicked" :disabled="isLoading" class="ml-0 action"
          >Approve</button-component
        >
        <button-component v-if="isApproved" @click="onUnApproveClicked" :disabled="isLoading" class="ml-0 action"
          >Remove approval</button-component
        >
        <button-component v-if="!isBlocked" @click="onBlockClicked" :disabled="isLoading" class="ml-0 action danger"
          >Block client</button-component
        >
        <button-component v-if="isBlocked" @click="onUnBlockClicked" :disabled="isLoading" class="ml-0 action danger"
          >Unblock client</button-component
        >
      </div>
          ...
          <div class="client-details block">
            <div>
              ...
              <div class="client-details-row">
                <div>UserAgent</div>
                <div>
                  <div class="truncate" @click="toggleTruncate">
                    {{ options.client.userAgent }}
                  </div>
                </div>
              </div>
              <div class="client-details-row">
                ...
                <div>
                  {{ formatDate(options.currentChallengeData.requestedAt) }}
                </div>
              </div>
              ...
              <div class="client-details-row" v-if="options.client.lastAttemptedAccessedAtUtc">
                ...
                <div>
                  {{ formatDate(options.client.lastAttemptedAccessedAtUtc) }}
                </div>
              </div>
              ...
              <div class="client-details-row mt-4">
                <div>
                  <a :href="`/api/ProxyClientIdentity/redirect/to-client-details/${options.client.id}`"
                    >[Go to client details]</a
                  >
                </div>
              </div>
            </div>
        ...
        <div>
          <div class="block-title">
            <div class="ipdetails-location">
              <div v-if="ipContinent" class="ipdetails-location__part">
                {{ ipContinent }}
              </div>
              ...
              <div v-if="ipCity" class="ipdetails-location__part">
                {{ ipCity }}
              </div>
            </div>
          </div>
      ...
      <div class="challenge-statuses">
        ...
        <div class="block block--dark">
          <div class="challenges" v-if="options.allChallengeData.length > 0">
            <div
              v-for="challenge in options.allChallengeData"
              class="challenge"
              :title="getChallengeTooltip(challenge)"
            >
              <div class="material-icons icon" :class="getChallengeClasses(challenge)">
