...
import { PlaceholderInfo, PlaceholderGroupInfo, getPlaceholdersForNotificationType, NotificationAlertTypeOptions, NotificationAlertTypeOption, NotificationTriggerOption, NotificationTriggerOptions } from "@utils/Constants";
...
export default class NotificationPage extends Vue {
	...
	webHookRequestData: CustomRequestData = { body: '', headers: '', requestMethod: 'GET', url: '' };
	...
	get showWebhook(): boolean { return this.rule.alertType == NotificationAlertType.WebHook; }
	...
	<div class="notification-page">
		...
		<div v-if="service.status.hasDoneAtLeastOnce">
			<div v-if="rule" class="block">
				...
				<div class="mt-2">
					...
					<select v-model="rule.triggerType" :disabled="isLoading">
						<option v-for="triggerType in notificationTriggerOptions" 
							:value="triggerType.value">{{ triggerType.name }}</option>
					</select>
				</div>
				...
				<div class="mt-2">
					...
					<select v-model="rule.alertType" :disabled="isLoading">
						<option v-for="alertType in notificationAlertTypeOptions" 
							:value="alertType.value">{{ alertType.name }}</option>
					</select>
				</div>
				
				<CustomRequestDataEditor v-if="showWebhook"
			...
			<div class="block mt-2 mb-2 pb-1" v-if="rule && (rule.lastNotifiedAtUtc || rule.lastNotifyResult)">
				<div v-if="rule.lastNotifiedAtUtc">Last notified at: <code>{{ formatDate(rule.lastNotifiedAtUtc) }}</code></div>
				<div v-if="rule.lastNotifyResult">Last notify result: <code class="overflow-x-scroll">'{{ rule.lastNotifyResult }}'</code></div>
			</div>
