	...
	<div class="blocked-ip-page">
		...
		<div v-if="service.status.hasDoneAtLeastOnce">
			<div v-if="rule" class="block">
				...
				<text-input-component label="Blocked until" v-model:value="rule.blockedUntilUtc" :disabled="isLoading" emptyIsNull="true" />

				<div class="block block--dark mt-4">
					<label class="block-title">Block type</label>
					<div class="mt-1 mb-2">
						<select v-model="rule.type" :disabled="isLoading">
							<option v-for="blockType in blockTypeOptions" 
								:value="blockType.value">{{ blockType.name }}</option>
						</select>
					</div>

					<text-input-component label="IP" v-model:value="rule.ip" v-if="rule.type == 'IP'" :disabled="isLoading" />
					<text-input-component label="IP RegEx" v-model:value="rule.ipRegex" v-if="rule.type == 'IPRegex'" :disabled="isLoading" />
					<text-input-component label="CIDR range" v-model:value="rule.cidrRange" v-if="rule.type == 'CIDRRange'" :disabled="isLoading" />
				</div>
				...
				<div class="block block--dark mt-4">
					...
					<div class="ip-test-result" :class="testResultClasses" v-if="testResult">{{ testResult }}</div>
