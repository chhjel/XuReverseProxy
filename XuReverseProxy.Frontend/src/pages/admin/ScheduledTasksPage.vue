    ...
    <div v-if="service.status.hasDoneAtLeastOnce">
      <div v-for="data in jobDatas" class="job block mb-4">
        ...
        <div class="job__chips" v-if="data.status">
          ...
          <div class="job__chip" v-if="data.status.lastStartedAt">
            Started at {{ formatDate(data.status.lastStartedAt) }}
          </div>
        ...
        <div
          ...
          v-if="data.status"
          :class="{
            success: !data.status.isRunning && !data.status.failed,
        ...
        <div
          class="job__result"
          v-if="data.result"
          :class="{ success: data.result.success, error: !data.result.success }"
        >
          <b>Last result</b>
          <div class="job__result-time">
            Ran from {{ formatDate(data.result.startedAtUtc) }} to
            {{ formatDate(data.result.stoppedAtUtc) }}
          </div>
          <div class="job__result-message">{{ data.result.result }}</div>
          <div class="job__result-exception" v-if="data.result.exception">
            <code>{{ data.result.exception }}</code>
          </div>
        </div>
      </div>
    </div>
