...
export default class EFCrudServiceBase<TEntity> extends ServiceBase {
    ...
    public async GetAllFullAsync(status: LoadStatus | null = null): Promise<GenericResultData<Array<TEntity>>> {
        ...
        const request = this.fetchExt(url, "GET", null);
        const result = await this.awaitWithStatus<GenericResultData<Array<TEntity>>>(request, status, true, 'GetAllFullAsync');
        return result.data;
    }
    ...
    public async CreateOrUpdateAsync(entity: TEntity, status: LoadStatus | null = null, loaderId: string | null = null): Promise<GenericResultData<TEntity>> {
        const url = `${this._baseUrl}/`;
        const request = this.fetchExt(url, "POST", entity);
        const result = await this.awaitWithStatus<GenericResultData<TEntity>>(request, status, true, loaderId || 'CreateOrUpdateAsync');
        return result.data;
    }

    public async DeleteAsync(entityId: string, status: LoadStatus | null = null, loaderId: string | null = null): Promise<GenericResult> {
        const url = `${this._baseUrl}/${entityId}`;
