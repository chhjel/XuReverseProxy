...
const routes: Readonly<RouteRecordRaw[]> = [
  { name: "/", path: "/", meta: { title: "" }, component: AdminOverviewPage },
  {
    name: "proxyconfigs",
    path: "/proxyconfigs",
    meta: { title: "Proxy Configs" },
    component: ProxyConfigsPage,
  },
  {
    name: "proxyconfig",
    path: "/proxyconfigs/:configId",
    meta: { title: "Proxy Config" },
  ...
  },
  {
    ...
    path: "/clients",
    meta: { title: "Clients" },
    component: ProxyClientsPage,
  },
  {
    name: "client",
    path: "/client/:clientId",
    meta: { title: "Client" },
    component: ProxyClientPage,
  },
  {
    name: "notifications",
    path: "/notifications",
  ...
  {
    name: "client-audit-log",
    path: "/client-audit-log",
    meta: { title: "Client Audit Log" },
    component: ClientAuditLogPage,
  },
  {
    name: "server-log",
    path: "/server-log",
...
export default function createAdminRouter(opts: AdminPageFrontendModel) {
  const serverName = opts.serverName || "XuReverseProxy";
  // Allow whitespace as servername to remove suffix
  const titleSuffix = serverName.trim().length == 0 ? "" : ` | ${serverName}`;

  adminRouter.afterEach((to: RouteLocationNormalized) => {
    void nextTick(() => {
