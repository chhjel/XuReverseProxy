...
export default class TimeSpanInputComponent extends Vue {
    ...
    emitLocalValue(): void
    {
        ...
        this.validateInput(() => this.localValueDays, v => this.localValueDays = v, 10000000);
        this.validateInput(() => this.localValueHours, v => this.localValueHours = v, 23);
        this.validateInput(() => this.localValueMinutes, v => this.localValueMinutes = v, 59);
        this.validateInput(() => this.localValueSeconds, v => this.localValueSeconds = v, 59);

        let valueToEmit: string | null = null
        if (this.emptyIsNull && `${this.localValueDays}${this.localValueHours}${this.localValueMinutes}${this.localValueSeconds}` === '') {
            valueToEmit = null;
        }
        else {
  ...
  <div class="input-wrapper" :class="wrapperClasses">
    ...
    <div class="timespan__inputs">
        ...
        <div class="timespan__clear">
            <button-component icon="close" :disabled="disabled" v-if="emptyIsNull"
                title="Clear" iconOnly secondary @click="clear" class="ml-2"></button-component>
        </div>
    </div>
