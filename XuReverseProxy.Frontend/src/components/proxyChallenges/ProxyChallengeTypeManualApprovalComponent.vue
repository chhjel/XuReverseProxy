...
export default class ProxyChallengeTypeManualApprovalComponent extends Vue {
  ...
  get hasRequested(): boolean {
    return this.options.hasRequested || this.result?.success == true;
  }
  ...
  get formattedEasyCode(): string {
    if (!this.options.easyCode || this.options.easyCode.length < 6) return this.options.easyCode;
      ...
      return `${this.options.easyCode.substring(0, 2)} ${this.options.easyCode.substring(
        2,
        4,
      )} ${this.options.easyCode.substring(4, 6)}`;
  }
...
<template>
  <div class="challenge-manual-approval">
    ...
    <div v-if="!hasRequested" class="challenge-manual-approval__inputs">
      <button-component @click="onRequestAccessClicked" :disabled="isLoading" class="ml-0 secondary"
