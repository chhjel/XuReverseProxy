...
export default class IPDetailsComponent extends Vue {
  ...
  get isValidIp(): boolean {
    return this.ip && !this.isLocalhost;
  }
  get ipContinent(): string | null {
    return this.ipLookupData?.continent || null;
  }
  get ipCountry(): string | null {
    return this.ipLookupData?.country || null;
  }
  get ipCity(): string | null {
    return this.ipLookupData?.city || null;
  }
  get ipFlagUrl(): string | null {
    return this.ipLookupData?.flagUrl || null;
  }
}
</script>
        ...
        <div class="block mb-4">
          ...
          <div v-if="canUnblockIp || !isIpBlocked">
            <checkbox-component
              ...
              :disabled="isLoading"
            />
            <div v-if="blockedIpNote" class="mt-2">
              <code>{{ blockedIpNote }}</code>
            </div>
          </div>
