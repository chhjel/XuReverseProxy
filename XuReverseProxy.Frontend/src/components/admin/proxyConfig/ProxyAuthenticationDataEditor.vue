...
<template>
  <div class="proxyconfigauth-edit" v-if="localValue">
    <select v-model="localValue.challengeTypeId" class="mb-2 mt-2" :disabled="disabled">
      <option v-for="challengeType in challengeTypeOptions" :value="challengeType.typeId">
        {{ challengeType.name }}
      </option>
    </select>

    <component
      :is="`${localValue.challengeTypeId}Editor`"
      :disabled="disabled"
    ...
    <div class="mt-3">
      <time-span-input-component
        ...
        emptyIsNull="true"
        v-model:value="localValue.solvedDuration"
        :disabled="disabled"
      />
    </div>
  </div>
